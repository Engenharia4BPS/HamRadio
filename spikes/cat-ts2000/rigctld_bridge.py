...
def _read_ini(path: str) -> Dict[str, object]:
    ...
    int_keys = {
        "baud": "baud",
        "keying_baud": "keying_baud",
        "radio_keying_baud": "radio_keying_baud",
        "rig_port": "rig_port",
        "poll_ms": "poll_ms",
    }
    bool_keys = {
    ...
    for ini_key, dest in string_keys.items():
        if ini_key in s and s.get(ini_key, "").strip():
            values[dest] = s.get(ini_key, "").strip()
    for ini_key, dest in int_keys.items():
        if ini_key in s and s.get(ini_key, "").strip():
            values[dest] = s.getint(ini_key)
    for ini_key, dest in bool_keys.items():
        if ini_key in s:
            values[dest] = s.getboolean(ini_key)
...
def parse_args() -> argparse.Namespace:
    ...
    bootstrap.add_argument("--config", default="bridge.ini")
    ...
    defaults.update(_read_ini(known.config))

    p = argparse.ArgumentParser(
        parents=[bootstrap],
        description="GADX Vector SPIKE: TS-2000 CAT + real-time serial keying bridge",
    )
    ...
    p.add_argument("--allow-write", action=argparse.BooleanOptionalAction)
    p.add_argument("--allow-ptt", action=argparse.BooleanOptionalAction)
    ...
    try:
        ...
        if args.keying_port:
            thread = threading.Thread(
                ...
                args=(args.keying_port, args.keying_baud, events, stop),
                daemon=True,
            )
            thread.start()
            ...
            while True:
                while True:
                    try:
                        ...
                        break
                    if event.kind == "ERROR":
                        raise serial.SerialException("keying input failed")
                    if event.kind == "PTT":
                    ...
                    if event.kind == "CW":
                        if event.enabled:
                            ...
                            pulse = (
                                (event.timestamp - cw_asserted_at) * 1000
                ...
                if not args.allow_write:
                    ...
                    if rf != bf:
                        LOG.warning("CAT -> PHYSICAL RADIO set frequency: %d Hz", rf)
                        rig.set_frequency(rf)
                    if rm != bm:
                        ...
                        if name in reverse:
                            LOG.warning("CAT -> PHYSICAL RADIO set mode: %s", reverse[name])
                            rig.set_mode(reverse[name], 0)
                    if rp != bp:
                        cat_ptt_requested = rp
                        update_ptt()
