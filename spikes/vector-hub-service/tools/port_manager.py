...
class Com0Com:
    ...
    def _interactive(self, commands: List[str], timeout: int = 30) -> str:
        ...
        try:
            proc = subprocess.run(
                [str(self.exe)], input=payload, text=True,
                stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                cwd=str(self.exe.parent), encoding="utf-8", errors="replace",
    ...
    def busy_names(self) -> Set[str]:
        return {
            line.strip().upper()
            for line in self._interactive(["busynames *"]).splitlines()
...
def active_ports() -> dict[str, str]:
    ...
    for item in list_ports.comports():
        ...
        if name:
            result[name] = item.description or item.hwid or "Porta serial"
    return result
...
class ProgressDialog(tk.Toplevel):
    ...
    def close(self):
        try:
            self.progress.stop(); self.grab_release(); self.destroy()
        except tk.TclError:
            pass
...
class ClientRow:
    def __init__(self, manager: "PortManagerApp", desired: DesiredClient):
        ...
        self.widgets = [self.name_entry, self.cat_type_combo, self.cat_app_combo, self.cat_arrow,
                        self.cat_vector_combo, self.key_type_combo, self.key_app_combo, self.key_arrow,
                        self.key_vector_combo, self.remove_button]
    ...
    def _refresh_combo(self, combo, side, current):
    ...
    def _sync_enabled_state(self):
        ...
        self.cat_app_combo.configure(state="readonly" if cat else "disabled")
        self.cat_vector_combo.configure(state="readonly" if cat else "disabled")
        self.key_app_combo.configure(state="readonly" if key else "disabled")
        self.key_vector_combo.configure(state="readonly" if key else "disabled")
    ...
    def desired_pairs(self):
        ...
        if self.cat_type.get() != "NONE":
            result.append(DesiredPair(name, "CAT", self.cat_app.get().strip().upper(), self.cat_vector.get().strip().upper()))
        if self.key_type.get() != "NONE":
            result.append(DesiredPair(name, "KEYING", self.key_app.get().strip().upper(), self.key_vector.get().strip().upper()))
        return result
...
class PortManagerApp(tk.Tk):
    def __init__(self):
        ...
        self.geometry("1120x650"); self.minsize(1040, 580)
        self.rows = []
    ...
    def _build(self):
        ...
        for col, size in {0:115,1:78,2:82,3:24,4:82,5:18,6:86,7:82,8:24,9:82,10:80}.items():
            self.table.grid_columnconfigure(col, minsize=size)
        ...
        ttk.Button(buttons, text="Carregar configuracao atual", command=self.load_current_config).pack(side="left", padx=(0,6))
        ...
        self.message_var = tk.StringVar(value="Fase C v0.6: pode carregar o vector.ini atual e cruza-lo com os pares com0com.")
        ttk.Label(self, textvariable=self.message_var, padding=(10, 4)).pack(fill="x")

    def _begin_work(self, title, worker: Callable, on_success: Callable, on_error=None):
    ...
    def _progress_status(self, status, detail=""):
        self.after(0, lambda:self._progress and self._progress.update_status(status, detail))

    def _collect_inventory(self):
        self._progress_status("Etapa 1 de 5 - Lendo portas seriais...", "Consultando dispositivos COM ativos no Windows")
        active = active_ports()
        self._progress_status("Etapa 2 de 5 - Localizando com0com...", "Procurando setupc.exe")
        com0com = Com0Com.discover()
        self._progress_status("Etapa 3 de 5 - Consultando pares virtuais...", str(com0com.exe))
        pairs = com0com.list_pairs()
        self._progress_status("Etapa 4 de 5 - Consultando portas reservadas...", "Lendo nomes registrados no ComDB")
        busy = com0com.busy_names()
        self._progress_status("Etapa 5 de 5 - Atualizando inventario...", f"{len(pairs)} par(es) virtual(is) encontrado(s)")
    ...
    def refresh_inventory(self, show_progress=False, initial=False):
        ...
        def ok(result):
            ...
            self._render_inventory(); self.refresh_row_choices()
            if initial and not self.existing_pairs and not CONFIG_PATH.exists():
                self.load_default_suggestion()
        def fail(exc):
            self.com0com = None; self.existing_pairs = []; self.active = active_ports(); self.busy = set()
            self.setupc_var.set(str(exc) if isinstance(exc, Com0ComTimeout) else f"com0com indisponivel: {exc}")
            self._render_inventory(); self.refresh_row_choices()
            if initial and not CONFIG_PATH.exists():
                self.load_default_suggestion()
        if show_progress:
            self._begin_work("Carregando inventario", self._collect_inventory, ok, fail)
        else:
    ...
    def _render_inventory(self):
        ...
        if self.existing_pairs:
            ...
            lines.extend(f"  #{p.index}: {p.app_port} <-> {p.vector_port}" for p in self.existing_pairs)
        else:
    ...
    def _other_end(self, vector_port: str) -> str:
        vector_port = vector_port.strip().upper()
        for pair in self.existing_pairs:
            if pair.app_port == vector_port:
                return pair.vector_port
            if pair.vector_port == vector_port:
                return pair.app_port
        return ""

    def load_current_config(self):
        if not CONFIG_PATH.exists():
            messagebox.showerror("Configuracao atual", f"Arquivo nao encontrado:\n{CONFIG_PATH}")
            return
        try:
            cfg = configparser.ConfigParser()
            cfg.read(CONFIG_PATH, encoding="utf-8-sig")
            if "cat" not in cfg:
                raise ValueError("vector.ini nao contem a secao [cat]")
            cat_ports = [p.strip().upper() for p in cfg["cat"].get("ports", "").split(",") if p.strip()]
            key_entries = []
            if "keying" in cfg:
                for key, value in cfg["keying"].items():
                    ...
                    if parts and parts[0]:
                        key_entries.append((int(match.group(1)), parts[0]))
                key_entries.sort()
            names = {}
            if "clients" in cfg:
                for key, value in cfg["clients"].items():
                    match = CLIENT_RE.match(key)
                    if match and value.strip():
                        names[int(match.group(1))] = value.strip()

            count = max(len(cat_ports), len(key_entries))
            if count == 0:
                raise ValueError("Nao ha clientes CAT ou KEYING no vector.ini")

            self.clear_rows()
            missing_app = []
            for index in range(count):
                cat_vector = cat_ports[index] if index < len(cat_ports) else ""
                key_vector = key_entries[index][1] if index < len(key_entries) else ""
                cat_app = self._other_end(cat_vector) if cat_vector else ""
                key_app = self._other_end(key_vector) if key_vector else ""
                if cat_vector and not cat_app: missing_app.append(cat_vector)
                if key_vector and not key_app: missing_app.append(key_vector)
                self.add_client_row(DesiredClient(
                    name=names.get(index + 1, f"Cliente {index + 1}"),
                    cat_type="CAT" if cat_vector else "NONE",
                    cat_app=cat_app,
                    cat_vector=cat_vector,
                    key_type="KEYING" if key_vector else "NONE",
                    key_app=key_app,
            ...
            if missing_app:
                self.message_var.set(
                    "vector.ini carregado; nao foi possivel descobrir a outra ponta de: " + ", ".join(missing_app)
                )
            else:
                self.message_var.set(f"Configuracao atual carregada de {CONFIG_PATH}")
        except Exception as exc:
            messagebox.showerror("Configuracao atual", str(exc))
    ...
    def port_choices(self, side, current="", owner=None):
        ...
        existing = self.existing_pair_ports(); selected = self.ports_selected_by_other_rows(owner)
        start, end = (APP_COM_MIN, APP_COM_MAX) if side == "app" else (VECTOR_COM_MIN, VECTOR_COM_MAX)
        choices = []
        for number in range(start, end + 1):
            port = f"COM{number}"
            occupied = port in self.active and port not in existing
            reserved = port in self.busy and port not in existing
            if not occupied and not reserved and port not in selected:
                choices.append(port)
    ...
    def add_client_row(self, desired):
        row = ClientRow(self, desired); self.rows.append(row); self.regrid_rows(); self.refresh_row_choices()
    ...
    def regrid_rows(self):
        for index, row in enumerate(self.rows):
            row.place(self._first_client_grid_row + index)

    def _next_free(self, start, used):
        existing = self.existing_pair_ports(); number = start
        while True:
            port = f"COM{number}"
            if port not in used and (port not in self.active or port in existing) and (port not in self.busy or port in existing):
                return port
            number += 1
    ...
    def load_default_suggestion(self):
        ...
        for name in ("LogHX", "N1MM"):
            cat_app = self._next_free(app, used); used.add(cat_app); app = com_number(cat_app) + 1
            cat_vector = self._next_free(vector, used); used.add(cat_vector); vector = com_number(cat_vector) + 1
            key_app = self._next_free(app, used); used.add(key_app); app = com_number(key_app) + 1
            key_vector = self._next_free(vector, used); used.add(key_vector); vector = com_number(key_vector) + 1
            self.add_client_row(DesiredClient(name, "CAT", cat_app, cat_vector, "KEYING", key_app, key_vector))
    ...
    def add_suggested_client(self):
        ...
        cat_app = self._next_free(APP_COM_MIN, used); used.add(cat_app)
        cat_vector = self._next_free(VECTOR_COM_MIN, used); used.add(cat_vector)
        key_app = self._next_free(APP_COM_MIN, used); used.add(key_app)
        key_vector = self._next_free(VECTOR_COM_MIN, used)
        self.add_client_row(DesiredClient(f"Cliente {len(self.rows)+1}", "CAT", cat_app, cat_vector, "KEYING", key_app, key_vector))
        ...
        for item in desired:
            ...
            for port in (item.app_port, item.vector_port):
                ...
                if port in self.busy and port not in existing:
                    return f"{port} esta reservada no ComDB. A v0.6 nao forca reservas/conflitos."
    ...
    def apply_plan(self):
        if not is_admin():
            messagebox.showerror("Permissao", "Execute o Port Manager como Administrador para alterar o com0com."); return
        if not self.com0com:
            messagebox.showerror("com0com", "com0com nao esta pronto. Use Recarregar inventario e verifique a mensagem exibida."); return
        desired = self.desired_pairs(); error = self.validate_plan(desired)
        if error:
            messagebox.showerror("Plano invalido", error); return
        current = {(p.app_port, p.vector_port): p for p in self.existing_pairs}
        wanted = {(d.app_port, d.vector_port) for d in desired}
        to_remove = [p for key, p in current.items() if key not in wanted]
        ...
        if not to_remove and not to_create:
            messagebox.showinfo("Sem alteracoes", "O com0com ja corresponde ao plano exibido."); return
        ...
        def worker():
            ...
            for pair in to_remove:
                self._progress_status(f"Operacao {done+1} de {total} - Removendo par...", f"{pair.app_port} <-> {pair.vector_port}")
                self.com0com.remove_pair(pair.index); done += 1
            for item in to_create:
                self._progress_status(f"Operacao {done+1} de {total} - Validando portas...", f"{item.app_port} <-> {item.vector_port}")
                active = active_ports(); busy = self.com0com.busy_names()
                if item.app_port in active or item.vector_port in active or item.app_port in busy or item.vector_port in busy:
                    raise RuntimeError(f"Conflito detectado antes de criar {item.app_port} <-> {item.vector_port}; operacao interrompida")
                self._progress_status(f"Operacao {done+1} de {total} - Criando par virtual...", f"{item.app_port} <-> {item.vector_port} ({item.name}/{item.kind})")
                output = self.com0com.create_pair(item.app_port, item.vector_port)
                if "ERROR:" in output.upper():
                    raise RuntimeError(output.strip())
                done += 1
            self._progress_status("Finalizando - Confirmando configuracao...", "Relendo o inventario do com0com")
        ...
        def ok(result):
            ...
            self.setupc_var.set(f"com0com: {self.com0com.exe}")
            self._render_inventory(); self.refresh_row_choices()
            self.message_var.set("Configuracao de portas aplicada. O vector.ini ainda nao e regravado automaticamente nesta versao.")
            messagebox.showinfo("Concluido", "Plano aplicado ao com0com. Nesta v0.6 o vector.ini ainda nao e alterado automaticamente.")

        def fail(exc):
            messagebox.showerror("Falha", str(exc)); self.refresh_inventory(True)
